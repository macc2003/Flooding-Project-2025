...
def resampling_continuous(run_title):
    ...
    # -- Store aligned rasters for optional visualization --
    resampled_rasters = {"Reference": ref_array}

    # -- Process and save each raster --
    ...
    # # -- Plotting (Optional) --
    # n_rasters = len(resampled_rasters)
    # n_cols = 3
    ...
    # axes = axes.ravel()
    #
    # for idx, (title, raster) in enumerate(resampled_rasters.items()):
    #     axes[idx].imshow(raster, cmap='terrain',
    #                      extent=[ref_bounds.left, ref_bounds.right, ref_bounds.bottom, ref_bounds.top])
    #     axes[idx].set_title(f"{title} Raster")
    #     axes[idx].set_xlabel("Longitude / Easting")
    #     axes[idx].set_ylabel("Latitude / Northing")
    #     axes[idx].axis('on')
    #
...
# -- Function to resample and align any raster to match the reference --
...
# -- Function to generate new filename with "_aligned" suffix --
def add_suffix(filename, suffix="_resampled"):
    name, ext = os.path.splitext(filename)
    return f"{name}{suffix}{ext}"

# -- Function to save resampled raster --
