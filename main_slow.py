...
with rasterio.open(f'{r_title}/produced/slope_adjusted_CN_{r_title}.tif') as src:
...
plt.imshow(slope_adj_CN)
plt.show()

with rasterio.open(f'{r_title}/produced/grassland_cn_{r_title}.tif') as src:
    # Read the image as a NumPy array (first band by default)
...
with rasterio.open(f'{r_title}/produced/et_water_loss_rate_{r_title}.tif') as src:
    # Read the image as a NumPy array (first band by default)
    ET_rate = src.read(1)  # Per 30s (so that flow speed is roughly 1m/s)


with rasterio.open(f'{r_title}/produced/usda_classification_{r_title}.tif') as src:
    # Read the image as a NumPy array (first band by default)
    usda_array = src.read(1)

with rasterio.open(f'{r_title}/produced/wilting_point_{r_title}.tif') as src:
    # Read the image as a NumPy array (first band by default)
    wilting_point_array = src.read(1)

with rasterio.open(f'{r_title}/produced/saturation_point_{r_title}.tif') as src:
    # Read the image as a NumPy array (first band by default)
    saturation_point_array = src.read(1)

with rasterio.open(f'{r_title}/produced/saturated_conductivity_{r_title}.tif') as src:
    # Read the image as a NumPy array (first band by default)
    saturated_conductivity = src.read(1)

with rasterio.open(f'{r_title}/resampled/fc_rpj_{r_title}_resampled.tif') as src:
    # Read the image as a NumPy array (first band by default)
    field_capacity_array = src.read(1)

field_capacity_array = field_capacity_array / 100

with rasterio.open(f'{r_title}/produced/manning_n_{r_title}.tif') as src:
    # Read the image as a NumPy array (first band by default)
    manning_n_array = src.read(1)

with rasterio.open(f'{r_title}/resampled/snowmelt_rpj_{r_title}_resampled.tif') as src:
    # Read the image as a NumPy array (first band by default)
    snowmelt_array = src.read(1)  # Note this is DAILY melt rate

with rasterio.open(f'{r_title}/produced/drainage_rate_{r_title}.tif') as src:
    # Read the image as a NumPy array (first band by default)
    drainage_array = src.read(1)

with rasterio.open(f'{r_title}/resampled/moisture_rpj_{r_title}_resampled.tif') as src:
    # Read the image as a NumPy array (first band by default)
    moisture_content_array = src.read(1)
...
accumulated_infiltration = moisture_content_array*1000
